...
/**
 * Default implementation of {@link cn.sexycode.myjpa.sql.dialect.pagination.LimitHandler} interface.
 *
 */
public abstract class AbstractLimitHandler implements LimitHandler {
    ...
    protected final int bindLimitParameters(RowSelection selection, PreparedStatement statement, int index)
            throws SQLException {
        if (!supportsVariableLimit() || !cn.sexycode.myjpa.sql.dialect.pagination.LimitHelper.hasMaxRows(selection)) {
            return 0;
        }
        final int firstRow = convertToFirstRowValue(cn.sexycode.myjpa.sql.dialect.pagination.LimitHelper.getFirstRow(selection));
