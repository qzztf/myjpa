...
import cn.sexycode.myjpa.query.criteria.internal.path.RootImpl.TreatedRoot;
...
public class QueryStructure<T> implements Serializable {
                ...
                if (correlationRoots != null) {
                    for (FromImplementor<?, ?> correlationRoot : correlationRoots) {
                        ...
                        for (Join<?, ?> correlationJoin : correlationRoot.getJoins()) {
                            ...
                            jpaqlQuery.append(correlationRootAlias).append('.')
                                    .append(correlationJoinImpl.getAttribute().getName()).append(" as ")
    ...
    @SuppressWarnings({ "unchecked" })
    private void renderJoins(StringBuilder jpaqlQuery, RenderingContext renderingContext,
    ...
    private void renderFetches(StringBuilder jpaqlQuery, RenderingContext renderingContext, Collection<Fetch> fetches) {
        ...
        for (Fetch fetch : fetches) {
            ...
            jpaqlQuery.append(renderJoinType(fetch.getJoinType())).append("fetch ")
