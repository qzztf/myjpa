package cn.sexycode.myjpa.type;

import cn.sexycode.myjpa.SessionFactoryObserver;
...
import cn.sexycode.sql.type.BasicType;
...
import cn.sexycode.sql.type.descriptor.java.JavaTypeDescriptorRegistry;
import cn.sexycode.sql.type.descriptor.sql.SqlTypeDescriptorRegistry;
...
public class TypeConfiguration implements SessionFactoryObserver, Serializable {
    ...
    public TypeConfiguration() {
        ...
        this.javaTypeDescriptorRegistry = new JavaTypeDescriptorRegistry();
        this.sqlTypeDescriptorRegistry = SqlTypeDescriptorRegistry.INSTANCE;
    ...
    public MetamodelImplementor scope(SessionFactoryImplementor sessionFactory) {
        ...
        for (Map.Entry<String, String> importEntry : scope.metadataBuildingContext.getMetadataCollector().getImports()
                .entrySet()) {
    ...
    public void addBasicTypeRegistrationContributions(List<BasicTypeRegistration> contributions) {
        for (BasicTypeRegistration basicTypeRegistration : contributions) {
            ...
            basicTypeRegistry.register(basicType, basicTypeRegistration.getRegistrationKeys());
            ...
            try {
                ...
                if (jdbcTypes.length == 1) {
                    ...
                    Set<String> hibernateTypes = jdbcToHibernateTypeContributionMap
                            .computeIfAbsent(jdbcType, k -> new HashSet<>());
    ...
    private static class Scope implements Serializable {
        ...
        public SessionFactoryImplementor getSessionFactory() {
            if (sessionFactory == null) {
                ...
                sessionFactory = (SessionFactoryImplementor) SessionFactoryRegistry.INSTANCE
                        .findSessionFactory(sessionFactoryUuid, sessionFactoryName);
                if (sessionFactory == null) {
                    throw new HibernateException(
                            "Could not find a SessionFactory [uuid=" + sessionFactoryUuid + ",name="
                                    + sessionFactoryName + "]");
                }
