...
*/
            ...
            if (StringUtils.isEmpty(columnName)) {
                if (propertyName != null) {
                    ...
                    Identifier implicitName = normalizer.normalizeIdentifierQuoting(
                            implicitNamingStrategy.determineBasicColumnName(new ImplicitBasicColumnNameSource() {
                                @Override
                    ...
                    if (implicitName.getText().contains("_collection&&element_")) {
                        implicitName = Identifier
                                .toIdentifier(implicitName.getText().replace("_collection&&element_", "_"),
    ...
    public Join getJoin() {
        ...
        if (join == null) {
            throw new AnnotationException(
                    "Cannot find the expected secondary table: no " + explicitTableName + " available for "
        ...
        if (formulaAnn != null) {
            ...
            if (actualCols == null) {
                ...
                for (int index = 0; index < length; index++) {
                    ...
                    final ImplicitNamingStrategy implicitNamingStrategy = context.getBuildingOptions()
                            .getImplicitNamingStrategy();
                    final PhysicalNamingStrategy physicalNamingStrategy = context.getBuildingOptions()
                    ...
                    column.setNullable(
                            col.nullable()); //TODO force to not null if available? This is a (bad) user choice.
    ...
    private void processExpression(ColumnTransformer annotation) {
        ...
        final String nonNullLogicalColumnName = logicalColumnName != null ? logicalColumnName
    ...
    private static Ejb3Column[] buildImplicitColumn(PropertyData inferredData, String suffixForDefaultColumnName,
        ...
        if (nullability != Nullability.FORCED_NULL && inferredData.getClassOrElement().isPrimitive() && !inferredData
                .getProperty().isArray()) {
    ...
    public String toString() {
        return "Ejb3Column" + "{table=" + getTable() + ", mappingColumn=" + mappingColumn.getName() + ", insertable="
                + insertable + ", updatable=" + updatable + ", unique=" + unique + '}';
    }
}
