...
import cn.sexycode.myjpa.metamodel.model.domain.spi.PluralPersistentAttribute;
 ...
 */
public abstract class AbstractPluralAttribute<D, C, E> extends AbstractAttribute<D, C>
    ...
    protected AbstractPluralAttribute(PluralAttributeBuilder<D, C, E, ?> builder) {
        super(builder.getDeclaringType(), builder.getProperty().getName(), builder.getAttributeNature(),
                builder.getValueType(), builder.getMember());

        this.collectionClass = builder.getCollectionClass();
    }

    public static <X, C, E, K> PluralAttributeBuilder<X, C, E, K> create(AbstractManagedType<X> ownerType,
            SimpleTypeDescriptor<E> attrType, Class<C> collectionClass, SimpleTypeDescriptor<K> keyType) {
