...
import cn.sexycode.myjpa.metamodel.model.domain.NavigableRole;
...
import cn.sexycode.sql.type.AssociationType;
...
public class MetamodelImpl implements MetamodelImplementor, Serializable {
    ...
    public void initialize(MetadataImplementor mappingMetadata,
        ...
        for (final PersistentClass model : mappingMetadata.getEntityBindings()) {
            ...
            final EntityPersister cp = persisterFactory
                    .createEntityPersister(model, accessStrategy, naturalIdAccessStrategy, persisterCreationContext);
            entityPersisterMap.put(model.getEntityName(), cp);

            if (cp.getConcreteProxyClass() != null && cp.getConcreteProxyClass().isInterface() && !Map.class
                    .isAssignableFrom(cp.getConcreteProxyClass()) && cp.getMappedClass() != cp
                ...
                if (cp.getMappedClass().equals(cp.getConcreteProxyClass())) {
                    ...
                    // specific test, but..
                    log.debugf("Entity [%s] mapped same interface [%s] as class and proxy", cp.getEntityName(),
                            cp.getMappedClass());
                } else {
                    ...
                    if (old != null) {
                        throw new HibernateException(String.format(Locale.ENGLISH,
                                ...
                                old, cp.getEntityName(), cp.getConcreteProxyClass().getName()));
                    }
                }
            }
        }
        ...
        for (final Collection model : mappingMetadata.getCollectionBindings()) {
            ...
            final CollectionDataAccess accessStrategy = sessionFactory.getCache()
                    .getCollectionRegionAccess(navigableRole);

            final CollectionPersister persister = persisterFactory
                    .createCollectionPersister(model, accessStrategy, persisterCreationContext);
        ...
        if (jpaMetaModelPopulationSetting != JpaMetaModelPopulationSetting.DISABLED) {
            MetadataContext context = new MetadataContext(sessionFactory,
                    mappingMetadata.getMappedSuperclassMappingsCopy(), jpaMetaModelPopulationSetting);
    ...
    private void primeSecondLevelCacheRegions(MetadataImplementor mappingMetadata) {
        ...
        for (PersistentClass bootEntityDescriptor : mappingMetadata.getEntityBindings()) {
            final AccessType accessType = AccessType
                    .fromExternalName(bootEntityDescriptor.getCacheConcurrencyStrategy());

            if (accessType != null) {
                if (bootEntityDescriptor.isCached()) {
                    regionConfigBuilders.computeIfAbsent(bootEntityDescriptor.getRootClass().getCacheRegionName(),
                            DomainDataRegionConfigImpl.Builder::new).addEntityConfig(bootEntityDescriptor, accessType);
                }

                if (bootEntityDescriptor instanceof RootClass && bootEntityDescriptor.hasNaturalId()
                        && bootEntityDescriptor.getNaturalIdCacheRegionName() != null) {
                    regionConfigBuilders.computeIfAbsent(bootEntityDescriptor.getNaturalIdCacheRegionName(),
        ...
        for (Collection collection : mappingMetadata.getCollectionBindings()) {
            ...
            if (accessType != null) {
                regionConfigBuilders
                        .computeIfAbsent(collection.getCacheRegionName(), DomainDataRegionConfigImpl.Builder::new)
    ...
    private void applyNamedEntityGraphs(java.util.Collection<NamedEntityGraphDefinition> namedEntityGraphs) {
        for (NamedEntityGraphDefinition definition : namedEntityGraphs) {
            log.debugf("Applying named entity graph [name=%s, entity-name=%s, jpa-entity-name=%s",
                    definition.getRegisteredName(), definition.getEntityName(), definition.getJpaEntityName());
            ...
            final RootGraphImpl entityGraph = new RootGraphImpl(definition.getRegisteredName(), entityType,
    ...
    private void applyNamedAttributeNodes(NamedAttributeNode[] namedAttributeNodes, NamedEntityGraph namedEntityGraph,
    ...
    private void applyNamedSubgraphs(NamedEntityGraph namedEntityGraph, String subgraphName,
    ...
    private static void registerEntityNameResolvers(EntityPersister persister,
    ...
    private static void registerEntityNameResolvers(EntityTuplizer tuplizer,
            Map<EntityNameResolver, Object> entityNameResolvers) {
    ...
    private static EntityTypeDescriptor<?> locateOrBuildEntityType(PersistentClass persistentClass,
    ...
    private static MappedSuperclassTypeImpl<?> buildMappedSuperclassType(MappedSuperclass mappedSuperclass,
        ...
        MappedSuperclassTypeImpl mappedSuperclassType = new MappedSuperclassTypeImpl(javaType, mappedSuperclass,
                superType, context.getSessionFactory());
    ...
    public String[] getImplementors(String className) throws MappingException {
        ...
        try {
            final Class<?> clazz = getSessionFactory().getServiceRegistry().getService(ClassLoaderService.class)
                ...
                if (isMappedClass) {
                    ...
                    if (mappedClass != null && clazz.isAssignableFrom(mappedClass)) {
                        ...
                        if (checkQueryable.isInherited()) {
                            Class<?> mappedSuperclass = entityPersister(checkQueryable.getMappedSuperclass())
                                    .getMappedClass();
