...
public class AttributeFactory {
    ...
    public <X, Y> PersistentAttributeDescriptor<X, Y> buildAttribute(ManagedTypeDescriptor<X> ownerType, Property property) {
        ...
        return new SingularAttributeImpl(ownerType, attributeMetadata.getName(), attributeMetadata.getJpaAttributeNature(),
                metaModelType, attributeMetadata.getMember(), false, false, property.isOptional());
    }
    ...
    public <X, Y> SingularPersistentAttribute<X, Y> buildIdAttribute(IdentifiableTypeDescriptor<X> ownerType, Property property) {
        ...
        final SingularAttributeMetadata<X, Y> attributeMetadata = (SingularAttributeMetadata) determineAttributeMetadata(
                wrap( ownerType, property ), identifierMemberResolver);

        return new SingularAttributeImpl.Identifier(ownerType, property.getName(), determineSimpleType( attributeMetadata.getValueContext() ),
                attributeMetadata.getMember(), attributeMetadata.getJpaAttributeNature());
    }
    ...
    public <X, Y> SingularAttributeImpl<X, Y> buildVersionAttribute(IdentifiableTypeDescriptor<X> ownerType, Property property) {
        ...
        final SingularAttributeMetadata<X, Y> attributeMetadata = (SingularAttributeMetadata<X, Y>) determineAttributeMetadata(
                wrap( ownerType, property ), versionMemberResolver);

        return new SingularAttributeImpl.Version(ownerType, property.getName(), attributeMetadata.getJpaAttributeNature(),
    ...
    private <X, Y, E, K> PersistentAttributeDescriptor<X, Y> buildPluralAttribute(PluralAttributeMetadata<X, Y, E> attributeMetadata) {
        final PluralAttributeBuilder info = new PluralAttributeBuilder(attributeMetadata.getOwnerType(),
                determineSimpleType( attributeMetadata.getElementValueContext() ), attributeMetadata.getJavaType(), java.util.Map.class.isAssignableFrom( attributeMetadata.getJavaType() )
                ? determineSimpleType( attributeMetadata.getMapKeyValueContext() )
                : null);

        return info.member( attributeMetadata.getMember() ).property( attributeMetadata.getPropertyMapping() ).persistentAttributeType( attributeMetadata.getJpaAttributeNature() )
                .build();
    }
    ...
    private <Y> SimpleTypeDescriptor<Y> determineSimpleType(ValueContext typeContext) {
        switch ( typeContext.getValueClassification() ) {
            ...
            case EMBEDDABLE: {
                ...
                final EmbeddedTypeDescriptor<Y> embeddableType = new EmbeddableTypeImpl<Y>(javaType,
                        typeContext.getAttributeMetadata().getOwnerType(),
                        (ComponentType) typeContext.getHibernateValue().getType(), context.getSessionFactory());
    ...
    private EntityMetamodel getDeclarerEntityMetamodel(AbstractIdentifiableType<?> ownerType) {
        ...
        if ( persistenceType == Type.PersistenceType.ENTITY ) {
            return context.getSessionFactory().getMetamodel().entityPersister( ownerType.getTypeName() ).getEntityMetamodel();
            ...
            return context.getSessionFactory().getMetamodel().entityPersister( persistentClass.getClassName() ).getEntityMetamodel();
        } else {
     ...
     */
    @SuppressWarnings({"unchecked"})
    private <X, Y> AttributeMetadata<X, Y> determineAttributeMetadata(AttributeContext<X> attributeContext,
        ...
        if ( type.isAnyType() ) {
            ...
            if ( type.isEntityType() ) {
                // entity
                return new SingularAttributeMetadataImpl<X, Y>(attributeContext.getPropertyMapping(), attributeContext.getOwnerType(),
                        member, determineSingularAssociationAttributeType( member ));
            }
            // collection
            if (value instanceof Collection) {
                ...
                return new PluralAttributeMetadataImpl(attributeContext.getPropertyMapping(), attributeContext.getOwnerType(),
                        member, persistentAttributeType, elementPersistentAttributeType, keyPersistentAttributeType);
...
//					);
            ...
            // component
            return new SingularAttributeMetadataImpl<X, Y>(attributeContext.getPropertyMapping(), attributeContext.getOwnerType(),
                    member, Attribute.PersistentAttributeType.EMBEDDED);
        } else {
            // basic type
            return new SingularAttributeMetadataImpl<X, Y>(attributeContext.getPropertyMapping(), attributeContext.getOwnerType(),
                    member, Attribute.PersistentAttributeType.BASIC);
        }
    ...
    private abstract class BaseAttributeMetadata<X, Y> implements AttributeMetadata<X, Y> {
        ...
        @SuppressWarnings({"unchecked"})
        protected BaseAttributeMetadata(Property propertyMapping, ManagedTypeDescriptor<X> ownerType, Member member,
...
//		throw new IllegalArgumentException( "Unexpected type [" + declaredType + "]" );
    ...
    private class SingularAttributeMetadataImpl<X, Y> extends BaseAttributeMetadata<X, Y> implements SingularAttributeMetadata<X, Y> {
        ...
        private SingularAttributeMetadataImpl(Property propertyMapping, ManagedTypeDescriptor<X> ownerType, Member member,
                Attribute.PersistentAttributeType persistentAttributeType) {
    ...
    private class PluralAttributeMetadataImpl<X, Y, E> extends BaseAttributeMetadata<X, Y> implements PluralAttributeMetadata<X, Y, E> {
        ...
        private PluralAttributeMetadataImpl(Property propertyMapping, ManagedTypeDescriptor<X> ownerType, Member member,
                Attribute.PersistentAttributeType persistentAttributeType, Attribute.PersistentAttributeType elementPersistentAttributeType,
            ...
            if ( keyPersistentAttributeType == null ) {
                elementJavaType = signatureType != null ? getClassFromGenericArgument( signatureType.getActualTypeArguments()[0] ) : Object.class; //FIXME and honor targetEntity?
                keyJavaType = null;
            } else {
                keyJavaType = signatureType != null ? getClassFromGenericArgument( signatureType.getActualTypeArguments()[0] ) : Object.class; //FIXME and honor targetEntity?
                elementJavaType = signatureType != null ? getClassFromGenericArgument( signatureType.getActualTypeArguments()[1] ) : Object.class; //FIXME and honor targetEntity?
            }
        ...
        private Class<?> getClassFromGenericArgument(java.lang.reflect.Type type) {
            if ( type instanceof Class ) {
                ...
                throw new AssertionFailure(
                        "Fail to process type argument in a generic declaration. Member : " + getMemberDescription() + " Type: " + type.getClass());
            }
        }
    ...
    private final MemberResolver embeddedMemberResolver = new MemberResolver() {
        ...
        public Member resolveMember(AttributeContext attributeContext) {
            ...
            final Getter getter = embeddableType.getHibernateType().getComponentTuplizer().getGetter( embeddableType.getHibernateType().getPropertyIndex( attributeName ) );
    ...
    private final MemberResolver virtualIdentifierMemberResolver = new MemberResolver() {
        ...
        public Member resolveMember(AttributeContext attributeContext) {
            ...
            final Getter getter = componentType.getComponentTuplizer().getGetter( componentType.getPropertyIndex( attributeName ) );
    ...
    private final MemberResolver normalMemberResolver = new MemberResolver() {
        ...
        public Member resolveMember(AttributeContext attributeContext) {
            ...
            } else if ( Type.PersistenceType.ENTITY == persistenceType || Type.PersistenceType.MAPPED_SUPERCLASS == persistenceType ) {
    ...
    private final MemberResolver identifierMemberResolver = new MemberResolver() {
        ...
        public Member resolveMember(AttributeContext attributeContext) {
            ...
            if ( PropertyAccessMapImpl.GetterImpl.class.isInstance( getter ) ) {
                return new MapMember(entityMetamodel.getIdentifierProperty().getName(),
    ...
    private final MemberResolver versionMemberResolver = new MemberResolver() {
        ...
        public Member resolveMember(AttributeContext attributeContext) {
            ...
            if ( PropertyAccessMapImpl.GetterImpl.class.isInstance( getter ) ) {
                return new MapMember(versionPropertyName,
