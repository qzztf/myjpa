...
import cn.sexycode.sql.sql.ordering.antlr.*;
    ...
    private static class TrimOperands {
        ...
        private TrimOperands(List<String> operands) {
            ...
            if (size == 1) {
                ...
                if (size < 1 || size > 4) {
                    throw new RuntimeException("Unexpected number of trim function operands : " + size);
                }
                ...
                if (!"from".equals(operands.get(size - 2))) {
                    throw new RuntimeException("Expecting FROM, found : " + operands.get(size - 2));
                }
