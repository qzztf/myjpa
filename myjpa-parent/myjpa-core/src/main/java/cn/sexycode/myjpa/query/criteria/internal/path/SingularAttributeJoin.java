...
import cn.sexycode.myjpa.query.criteria.internal.CriteriaSubqueryImpl;
import cn.sexycode.myjpa.query.criteria.internal.FromImplementor;
...
public class SingularAttributeJoin<O, X> extends AbstractJoinImpl<O, X> {
    ...
    public SingularAttributeJoin(CriteriaBuilderImpl criteriaBuilder, Class<X> javaType, PathSource<O> pathSource,
            SingularAttribute<? super O, ?> joinAttribute, JoinType joinType) {
        ...
        if (Attribute.PersistentAttributeType.EMBEDDED == joinAttribute.getPersistentAttributeType()) {
            ...
            if (javaType != null) {
                this.model = (Bindable<X>) criteriaBuilder.getEntityManagerFactory().getMetamodel()
    ...
    protected FromImplementor<O, X> createCorrelationDelegate() {
        return new SingularAttributeJoin<O, X>(criteriaBuilder(), getJavaType(), getPathSource(), getAttribute(),
                getJoinType());
    }
        ...
        if (getModel().getBindableType() == Bindable.BindableType.ENTITY_TYPE) {
            ...
            if (!ManagedType.class.isInstance(joinedAttributeType)) {
                throw new UnsupportedOperationException(
                        "Cannot further dereference attribute join [" + getPathIdentifier()
                                + "] as its type is not a ManagedType");
            }
            ...
            if (!ManagedType.class.isInstance(elementType)) {
                throw new UnsupportedOperationException(
                        "Cannot further dereference attribute join [" + getPathIdentifier()
                                + "] (plural) as its element type is not a ManagedType");
            }
