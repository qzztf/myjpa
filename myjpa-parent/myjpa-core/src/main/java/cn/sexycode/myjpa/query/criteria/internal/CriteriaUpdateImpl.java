...
import cn.sexycode.myjpa.query.criteria.internal.path.SingularAttributePath;
    ...
    protected <Y> void addAssignment(Path<Y> attributePath, Expression<? extends Y> value) {
        if (!PathImplementor.class.isInstance(attributePath)) {
            throw new IllegalArgumentException(
                    "Unexpected path implementation type : " + attributePath.getClass().getName());
        }
        if (!SingularAttributePath.class.isInstance(attributePath)) {
            throw new IllegalArgumentException("Attribute path for assignment must represent a singular attribute ["
    ...
    private void renderAssignments(StringBuilder jpaql, RenderingContext renderingContext) {
        ...
        try {
            ...
            for (Assignment assignment : assignments) {
                ...
                jpaql.append(assignment.attributePath.render(renderingContext)).append(" = ")
