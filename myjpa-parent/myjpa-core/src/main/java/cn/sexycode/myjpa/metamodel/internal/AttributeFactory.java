...
public class AttributeFactory {
	...
	public <X, Y> PersistentAttributeDescriptor<X, Y> buildAttribute(ManagedTypeDescriptor<X> ownerType,
		...
		final AttributeMetadata<X, Y> attributeMetadata = determineAttributeMetadata(attributeContext,
		...
		return new SingularAttributeImpl(ownerType, attributeMetadata.getName(),
				attributeMetadata.getJpaAttributeNature(), metaModelType, attributeMetadata.getMember(), false, false,
				property.isOptional());
	}
	...
	private <X, Y, E, K> PersistentAttributeDescriptor<X, Y> buildPluralAttribute(
			PluralAttributeMetadata<X, Y, E> attributeMetadata) {
		final PluralAttributeBuilder info = new PluralAttributeBuilder(attributeMetadata.getOwnerType(),
				determineSimpleType(attributeMetadata.getElementValueContext()), attributeMetadata.getJavaType(),
				java.util.Map.class.isAssignableFrom(attributeMetadata.getJavaType()) ? determineSimpleType(
						attributeMetadata.getMapKeyValueContext()) : null);

		return info.member(attributeMetadata.getMember()).property(attributeMetadata.getPropertyMapping())
				.persistentAttributeType(attributeMetadata.getJpaAttributeNature()).build();
	}
...
//			return (Class<Y>) Byte.class;
	...
	private class SingularAttributeMetadataImpl<X, Y> extends BaseAttributeMetadata<X, Y>
			implements SingularAttributeMetadata<X, Y> {
		private final ValueContext valueContext;

		private SingularAttributeMetadataImpl(Property propertyMapping, ManagedTypeDescriptor<X> ownerType,
...
//					return ((Collection) getPropertyMapping().getValue()).getElement();
	...
	/*private final MemberResolver embeddedMemberResolver = new MemberResolver() {
		...
		public Member resolveMember(AttributeContext attributeContext) {
			...
			final Getter getter = embeddableType.getHibernateType().getComponentTuplizer()
