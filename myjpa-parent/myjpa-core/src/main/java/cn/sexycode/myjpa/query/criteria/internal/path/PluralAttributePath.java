...
public class PluralAttributePath<X> extends AbstractPathImpl<X> implements Serializable {
        ...
        switch (attribute.getDeclaringType().getPersistenceType()) {
            ...
            case MAPPED_SUPERCLASS: {
                ...
                if (getPathSource().getModel().getBindableType() == Bindable.BindableType.ENTITY_TYPE) {
                    ...
                    throw new AssertionFailure(String.format("Unexpected BindableType; expected [%s]; instead got [%s]",
                            Bindable.BindableType.ENTITY_TYPE, getPathSource().getModel().getBindableType()));
                }
            }
            case EMBEDDABLE: {
                ...
                if (singularAttribute.getDeclaringType().getPersistenceType() == Type.PersistenceType.ENTITY) {
                ...
                } else if (singularAttribute.getDeclaringType().getPersistenceType()
                    ...
                    throw new AssertionFailure(String.format("Unexpected PersistenceType: [%s]",
    ...
    public <T extends X> PluralAttributePath<T> treatAs(Class<T> treatAsType) {
        throw new UnsupportedOperationException(
                "Plural attribute path [" + getPathSource().getPathIdentifier() + '.' + attribute.getName()
                        + "] cannot be dereferenced");
    }

    private EntityType locateNearestSubclassEntity(MappedSuperclassType mappedSuperclassType,
            EntityType entityTypeTop) {
        ...
        while (superType != mappedSuperclassType) {
            if (superType == null) {
                throw new IllegalStateException(String.format(
                        ...
                        mappedSuperclassType.getJavaType().getName(), entityTypeTop.getJavaType().getName(),
