...
import cn.sexycode.myjpa.sql.util.RowVersionComparator;
...
public class RowVersionTypeDescriptor extends AbstractTypeDescriptor<byte[]> {
    ...
    public <X> byte[] wrap(X value, WrapperOptions options) {
        ...
        if (InputStream.class.isInstance(value)) {
            return cn.sexycode.myjpa.sql.type.descriptor.java.DataHelper.extractBytes((InputStream) value);
        }
        if (Blob.class.isInstance(value) || cn.sexycode.myjpa.sql.type.descriptor.java.DataHelper.isNClob(value.getClass())) {
            try {
